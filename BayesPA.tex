...
\author{\name Tianlin Shi \email tianlinshi@gmail.com \\
...
\caption{Graphical Illustration of BayesPA learning. \textbf{(a)}. Update \emph{passively} by Bayes rule, if the resulting distribution suffer zero loss. \textbf{(b)} Otherwise, \emph{aggressively} project the resulting distribution to the feasible zone of weights with zero loss. }\label{fig:bayesPA}\vspace{-.2cm}
\end{figure}
...
\hline\hline
\end{tabular}
\caption{The comparison between BayesPA and its various precursors, including online PA, streaming variational Bayes (SVB) and regularized Bayesian inference (RegBayes), in three different aspects.}
...
\end{center}\vspace{-.2cm}
\caption{Graphical illustrations of: (a) the abstraction of models with latent structures; and (b) the procedure of BayesPA learning with latent structures.}\label{fig:LatentBayesPA}\vspace{-.2cm}
\end{figure}
...
\begin{figure}[t]
\begin{center}
\includegraphics[width = \textwidth]{plot_multic_commit_lda_jmlr.pdf}
\includegraphics[width = 0.97\textwidth]{plot_multic_commit_hdp_jmlr.pdf}
\end{center}
...
\begin{figure}[t]
\includegraphics[width = \textwidth]{plot_acc_topic_multic_lda.pdf}
\includegraphics[width = \textwidth]{plot_acc_topic_multic_hdp.pdf}%\vspace*{-.2cm}
...
\end{tabular} }
\end{center}
\caption{Visualization of the learnt topics by \paMedLDAave and \paMedLDAgibbs. See text for details.\junx{pdf files need cut off boundary.}}\vspace{-.2cm}
...
\begin{figure}
\includegraphics[width = \textwidth]{plot_mtask_commit_jmlr.pdf}
%\vspace*{-0.5cm}
...
\paragraph{Number of Topics.} We observe an accuracy degradation when running one-pass BayesPA with large number of topics on small datasets. 	To see this effect, we test on a very small binary classification dataset: Two groups $alt.atheism$ and $talk.religion.misc$ from the 20 newsgroup dataset are used for the binary classification. The training set contains 1597 documents with a split of 795/802 over two categories and the test set contains 401 documents with a split of 204/197.  For simplicity, we investigate with Gibbs MedLDA with the hyper-parameters as in the multi-class setting.

We believe this phenomenon is due to the fact that with limited data points and large number of parameters, BayesPA fails to converge in just one pass. We have came up with two practical approaches to mitigate this issue:

\begin{figure*}
	\includegraphics[width=\textwidth]{plot_degradation_topic}
	\caption{A performance degradation issue observed when running BayesPA on small data with large number of topics}
\end{figure*}

\begin{itemize}
	\item Simply try more passes. However, it takes more computational resources to do so.
	\item Create ghost copies of the data. For every data point $(\xv_d, y_d)$, make $L$ exact clones. Then in our global update equation, each aggregation of sufficient statistics will be multiplied by an extra factor of $L$. Notice this approach does not need more computation.
\end{itemize}

Both approaches turned out to be effective, as shown in Figure ~\ref{fg:solve_degradation}.

\begin{figure*}
	\includegraphics[width=\textwidth]{plot_solve_degradation}
	\caption{Solving the performance degradation issue with a couple of practical approaches: 1. ghost copies with $L = 10$. 2. total of five passes over the dataset.}
